...
def handler(event, context):
    ...
    vcert_bin_path = event.get("vcertBinPath", "/usr/local/bin/vcert")
    certificate_ids_to_process = event.get("certificateIds", [])
    token_switch = event.get("tokenSwitch", "-k")
    api_secret_region = event.get("apiSecretRegion", "us-east-1")
    aws_regions = event.get(
        "awsRegions",
        [
            "us-east-1",
            "us-west-2",
    ...
    standard_cross_account_role_name = event.get(
        "crossAccountRoleName", "CrossAccountSecretsAndACMRole"
    )
    #
    # Get api secret from secrets manager
    # Headers for api will change as iDP integration is added
    api_secrets = fetch_aws_secret("pki-tppl-api-key", region_name=api_secret_region)
    if not api_secrets:
        logger.error("Failed to retrieve API secrets. Exiting.")
        return {"statusCode": 500, "body": "API secrets missing"}
    api_token = api_secrets["tppl-api-key"]
    headers = {"tppl-api-key": f"{api_token}", "accept": "application/json"}
    #
    if certificate_ids_to_process:
        logger.info(
            f"Processing specific certificate IDs: {certificate_ids_to_process}"
        )
    ...
    if not certs_list:
        ...
        return {"statusCode": 200, "body": "No new certificates found"}

    # Fetch apps
    logger.info("Fetching AWS applications data...")
    app_id_to_app_name = {
        app["id"]: app["app"]
        for app in fetch_aws_applications_data(api_base_url, headers)
    }
    ...
    unique_app_ids_from_certs_list = {
        ...
        if app_id in app_id_to_app_name
    }
    logger.info(
    ...
    if not aws_account_number_to_credentials:
        ...
        return {"statusCode": 500, "body": "No AWS account credentials obtained."}
